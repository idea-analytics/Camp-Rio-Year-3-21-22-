...
Participants <- left_join(df_all_cr_schools, Stu_demg, 
                          by = c("SchoolShortName" = "SchoolShortName",
                                 "GradeLevelID" = "GradeLevelID"))
#write.csv(Participants, "Participants.csv")

Rise_participants <- left_join(df_all_cr_schools, Rise_stu, 
                               by = c("SchoolShortName" = "SchoolShortName",
                                      "mergeVar" = "mergeVar")) %>%
  filter(StudentNumber != "") %>%
  select(-GradeLevelID.x) %>%
  rename(GradeLevelID = GradeLevelID.y)
