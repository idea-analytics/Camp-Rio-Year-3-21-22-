...
# Inputs: [RGVPDRA-DASQL].[Dashboard].[dbo].[APResult]                         #
#         [RGVPDRA-DASQL].[Dashboard].[dbo].[APCode]                           #
...
# Get AP results
ap_result_conn_current_year <- get_table(.table_name = "APResult", 
                                         .database_name = "Dashboard",
                                         .schema = "dbo", 
                                         .server_name = "791150-HQVRA") %>%
  filter(BestScore == "TRUE", 
         SchoolYear == "2021-2022") %>%
  select(StudentNumberDelta, 
         ExamCode, 
         APResult = ExamGrade, 
         AdminYear, 
         GradeLevel, 
...
ap_result_conn_last_year <- get_table(.table_name = "APResult", 
                                      .database_name = "Dashboard", 
                                      .schema = "dbo", 
                                      .server_name = "791150-HQVRA") %>%
  ...
  group_by(StudentNumberDelta, 
           ...
           AdminYear, 
           GradeLevel) %>%
  summarize(APResult = max(ExamGrade)) %>%
...
ap_result_conn_all_prior_year <- get_table(.table_name = "APResult", 
                                           .database_name = "Dashboard", 
                                           .schema = "dbo", 
                                           .server_name = "791150-HQVRA") %>%
  filter(!(SchoolYear %in% c("2020-2021", "2021-2022"))) %>%
  group_by(StudentNumberDelta, 
           ExamCode, 
           AdminYear, 
           GradeLevel) %>%
...
# Get AP exam codes
ap_code_conn <- get_table(.table_name = "APCode", 
                          .database_name = "Dashboard", 
                          .schema = "dbo", 
                          .server_name = "791150-HQVRA") %>%
  select(ExamCode, 
